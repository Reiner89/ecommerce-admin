...
export const SAuxiliar = () => {
  const { gestSelect, seleccionarAuxiliar, selectAux, auxiliares } = useStock();
          ...
          <div className="relative text-xs flex flex-col gap-3">
            {auxiliares.map((dp) => (
              <p
                ...
                onClick={() => seleccionarAuxiliar(dp.name)}
              >
                {dp.name}
              </p>
            ))}
