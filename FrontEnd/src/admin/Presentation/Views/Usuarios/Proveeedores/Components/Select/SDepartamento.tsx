...
export const SDepartamento = () => {
  const { selectDpt, SeleccionarDpt, departamentos } = useAdministracion();
      ...
      <div className="relative text-xs flex flex-col gap-3">
        {departamentos.map((dp) => (
          <p
            ...
            onClick={() => SeleccionarDpt(dp.id, dp.name)}
          >
            {dp.name}
          </p>
        ))}
